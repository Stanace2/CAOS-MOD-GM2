function scr_dos_normal()
{
    ...
    else
    {
        ...
        if ((!machslideAnim) && (!landAnim) && (!shotgunAnim))
        {
            ...
            if (idle >= 150 && (!global.panic) && global.combo < 25 && breakdance_pressed < breakdance_max)
            {
                ...
                if (((!global.pistol) || (!ispeppino)) && sprite_index != spr_snapnailit && sprite_index != spr_judge && sprite_index != spr_player_freezeridle && sprite_index != spr_donisha_scared && (!shotgunAnim) && sprite_index != spr_idle1 && sprite_index != spr_idle2 && sprite_index != spr_idle3 && sprite_index != spr_idle4 && sprite_index != spr_idle5 && sprite_index != spr_idle6)
                {
            ...
            else if (!facehurt)
            {
                if (windingAnim < 1800 || angry || global.playerhealth == 1 || shoot)
                {
                    start_running = 1
                    movespeed = 0
                    if (sprite_index == spr_snapnailit)
                    {
                    }
                    else if shoot
        ...
        if ((key_jump || (input_buffer_jump > 0 && (!key_attack) && vsp > 0)) && (!key_down))
        {
            ...
            if (sprite_index != spr_shotgunshoot)
            {
                ...
                if shotgunAnim
                    ...
                    sprite_index = spr_dos_pistoljump1
                image_index = 0
            }
    ...
    else if (!key_jump)
    {
        if (sprite_index != spr_shotgunshoot)
        {
            ...
            if (global.pistol && ispeppino)
                sprite_index = spr_dos_pistoljump2
            image_index = 0
            jumpAnim = 0
        }
        else
