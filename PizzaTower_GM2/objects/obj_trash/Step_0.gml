...
switch (state)
{
	case states.normal:
		with (obj_player)
		{
			if (other.state == states.normal && state != states.trashstart && other.trashbuffer <= 0 && place_meeting(x, y, other))
			{
	...
	case states.trashstart:
		with (playerid)
		{
			if (state == states.trashstart && ANIMATION_END)
			{
